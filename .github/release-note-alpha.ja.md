<!--
  日本語版。**英語版 `.github/release-note-alpha.md` と対です。**
  **片方だけ直さないでください** —— 版ごとに書き換えるので、そこが必ずずれます。
-->
...
**実運用のサーバーで使われ始めました。**Issue #8〜#25 は、そこで出たものです。
...
| `sshboard.app.tar.gz` | macOS。展開して `/Applications` へ |
...
- **Windows の実機で使われています。**Issue #8〜#25 は、そこで出たものです。
  **ssh-agent（名前付きパイプ / Pageant）は未確認**で、いまはパスワードと鍵ファイルで繋がれています
