...
5. **`v0.1.0-alpha.1` のタグ**（人）。**押すと公開 Release が作られ、
   `.msi` / `-setup.exe` / `.app.zip` が添付されます**（D32）。
   添付ロジック（`ditto` の zip 化・拡張子での選別）は**手元で実物を使って通し済み**ですが、
   **`release` ジョブ自体はタグを押すまで一度も走りません。**ここは初回です
...
- **`.dmg` の生成が落ちます**（2026-09-02・手元）。`bundle_dmg.sh` が失敗しました。
  ...
  **署名を入れるときに dmg へ戻ります**
- ~~Windows の `.msi` / `.nsis` はまだどこでも束ねていません~~ — **2026-09-02 に CI で成功。**
  **ただし、束ねられることと、入れて動くことは別です。**
