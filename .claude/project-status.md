...
4. **`workflow_dispatch` で release を手動実行**（人）。
   タグを押す前に、**Windows で束ねられるか**を見るため。
   `.msi` / `.nsis` は**まだどこでも束ねていません**
...
- **`.dmg` の生成が落ちます**（2026-09-02・手元）。`bundle_dmg.sh` が失敗しました。
  ...
  **署名を入れるときに dmg へ戻ります**
- **Windows の `.msi` / `.nsis` は、まだどこでも束ねていません。**
  `tauri build` を通したのは macOS だけです（`.app` は出来ました）
