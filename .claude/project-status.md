...
`.claude/decisions.md`（D1〜D30）＋ `DESIGN.md`。
**未決は D10（実装体制）と D15（MCP のポート）。**

**D15 は α の使い勝手に直接効きます。**いまは OS に空きを選ばせているので、
**起動するたびにポートが変わり、`claude mcp add` をやり直す**ことになります。
AI からは提案までにして、**決めるのは人**（product-baseline §15）。
