...
<style>
	 ...
	 */
	/*
	 * **`.bar` はここだけ。**`display: flex` を足さないこと。
	 *
	 * 2026-09-18、`.sync` の重複をまとめたときに**この並びへ巻き込み**、
	 * `.bar` まで flex 容器になりました。すると中の芯（白）が
	 * **中身の幅しか取らなくなり、右に外殻の灰色が 291px 出ました。**
	 *
	 * 運用者の指摘は最初からここでした ——「**背景のグレーとズレているでしょ**」。
	 * **私は文字の詰まりばかり見ていて、形が揃っていないことを見ていませんでした。**
	 * 画素を読んで初めて数字になりました（y=90 で白が x=792 で切れ、以降が灰）。
	 */
	.bar {
		flex: 0 0 auto;
	}
