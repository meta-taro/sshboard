...
<style>
	.icon {
		/* **潰さない。**狭い行で字形が歪むより、はみ出す方がまだ読めます。 */
		flex: none;
		/*
		 * **`block` にしない**（Issue #23）。
		 *
		 * `block` は「親が flex である」ことを前提にした書き方でした。
		 * flex の中では正しく並びますが、**flex でない親（`<button>` など）では
		 * 単独のブロックになり、隣の文字を次の行へ押し出します。**
		 *
		 * 実機では［ファイルを選ぶ］がこうなりました。
		 *
		 *     [□]              ← アイコンが 1 行目
		 *      ファイルを選ぶ   ← ラベルが 2 行目（枠からはみ出す）
		 *
		 * アイコン入りのボタンを持つ部品は 4 つあり、**2 つだけが親を flex に
		 * していました。**当たった 2 か所へ `display: flex` を足す直し方は
		 * 採りません —— **次にアイコン入りのボタンを書いた人が、同じ穴に落ちます。**
		 *
		 * `inline-block` なら、flex の外では文字の隣に来ます。
		 * **flex の中では flex item が block 化されるので、並びは変わりません。**
		 */
		display: inline-block;
		/* 文字の中心へ寄せる。ベースラインに乗せると、字より下がって見えます。 */
