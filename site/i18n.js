...
window.SSHBOARD_I18N = {
  ru: {
    ...
    'f.releases': 'Релизы',
    'f.issues': 'Issues',
    'h.family': "Та же мысль, в другом месте",
    'family.intro': "Три инструмента, одно правило: <b>агент работает там, где вы его видите, и только внутри ограды, которую провели вы.</b> Всё бесплатно, всё с открытым кодом, всё на вашей машине — <b>регистрироваться негде.</b>",
    'family.db.sub': "Базы данных",
    'family.db': "Клиент баз данных со встроенным MCP-сервером — девять адаптеров, от MySQL и PostgreSQL до Turso, D1 и Firestore. <b>Только чтение, пока вы явно не откроете соединение</b>, и даже тогда выдача прав, DDL ролей, <code>TRUNCATE</code> и <code>DROP</code> отклоняются, что бы вы ни выставили.",
    'family.wa.sub': "Разговоры",
    'family.wa': "Люди и агенты в одном разговоре, напрямую между машинами. <b>Ни центрального аккаунта, ни сервера</b> — входит тот, кому вы дали ключ.",
  },
  ...
  it: {
    ...
    'f.releases': 'Release',
    'f.issues': 'Issues',
    'h.family': "La stessa idea, altrove",
    'family.intro': "Tre strumenti, una regola: <b>l'agente lavora dove puoi vederlo, e solo dentro una recinzione che hai tracciato tu.</b> Tutti gratuiti, tutti open source, tutti sulla tua macchina — <b>non c'è nessun posto dove iscriversi.</b>",
    'family.db.sub': "Database",
    'family.db': "Un client per database con server MCP integrato — nove adattatori, da MySQL e PostgreSQL a Turso, D1 e Firestore. <b>Sola lettura finché non apri esplicitamente una connessione</b>, e anche allora permessi, DDL dei ruoli, <code>TRUNCATE</code> e <code>DROP</code> vengono rifiutati qualunque cosa tu imposti.",
    'family.wa.sub': "Conversazioni",
    'family.wa': "Persone e agenti nella stessa conversazione, da macchina a macchina. <b>Nessun account centrale e nessun server da tenere acceso</b> — entra chi riceve la chiave.",
  },
  'pt-BR': {
    ...
    'f.releases': 'Versões',
    'f.issues': 'Issues',
    'h.family': "A mesma ideia, em outro lugar",
    'family.intro': "Três ferramentas, uma regra: <b>o agente trabalha onde você consegue vê-lo, e só dentro de uma cerca que você traçou.</b> Todas gratuitas, todas de código aberto, todas na sua própria máquina — <b>não há onde se cadastrar.</b>",
    'family.db.sub': "Bancos de dados",
    'family.db': "Um cliente de banco de dados com servidor MCP embutido — nove adaptadores, de MySQL e PostgreSQL a Turso, D1 e Firestore. <b>Somente leitura até você abrir uma conexão explicitamente</b>, e mesmo assim permissões, DDL de papéis, <code>TRUNCATE</code> e <code>DROP</code> são recusados independentemente do que você configure.",
    'family.wa.sub': "Conversas",
    'family.wa': "Pessoas e agentes na mesma conversa, de máquina para máquina. <b>Sem conta central e sem servidor para manter</b> — entra quem recebe a chave.",
  },
  es: {
    ...
    'f.issues': 'Issues',
    'h.family': "La misma idea, en otro sitio",
    'family.intro': "Tres herramientas, una regla: <b>el agente trabaja donde puedes verlo, y solo dentro de una valla que tú trazaste.</b> Todas gratuitas, todas de código abierto, todas en tu propia máquina — <b>no hay dónde registrarse.</b>",
    'family.db.sub': "Bases de datos",
    'family.db': "Un cliente de base de datos con servidor MCP incorporado — nueve adaptadores, de MySQL y PostgreSQL a Turso, D1 y Firestore. <b>Solo lectura hasta que abras una conexión de forma explícita</b>, y aun así los permisos, el DDL de roles, <code>TRUNCATE</code> y <code>DROP</code> se rechazan pongas lo que pongas.",
    'family.wa.sub': "Conversaciones",
    'family.wa': "Personas y agentes en una misma conversación, de máquina a máquina. <b>Sin cuenta central y sin servidor que mantener</b> — entra aquel a quien le das la llave.",
  },
  fr: {
    ...
    'f.releases': 'Versions',
    'f.issues': 'Issues',
    'h.family': "La même idée, ailleurs",
    'family.intro': "Trois outils, une règle : <b>l'agent travaille là où vous le voyez, et seulement dans une clôture que vous avez tracée.</b> Tous gratuits, tous ouverts, tous sur votre propre machine — <b>nulle part où s'inscrire.</b>",
    'family.db.sub': "Bases de données",
    'family.db': "Un client de base de données avec serveur MCP intégré — neuf adaptateurs, de MySQL et PostgreSQL à Turso, D1 et Firestore. <b>En lecture seule tant que vous n'avez pas explicitement ouvert une connexion</b>, et même alors les droits, le DDL de rôles, <code>TRUNCATE</code> et <code>DROP</code> sont refusés quoi que vous régliez.",
    'family.wa.sub': "Conversations",
    'family.wa': "Des humains et des agents dans une même conversation, de machine à machine. <b>Pas de compte central, pas de serveur à faire tourner</b> — celui à qui vous remettez la clé entre.",
  },
  de: {
    ...
    'f.releases': 'Releases',
    'f.issues': 'Issues',
    'h.family': "Dieselbe Idee, woanders",
    'family.intro': "Drei Werkzeuge, eine Regel: <b>Der Agent arbeitet dort, wo Sie ihn sehen, und nur innerhalb eines Zauns, den Sie gezogen haben.</b> Alle kostenlos, alle quelloffen, alle auf Ihrem eigenen Rechner — <b>nirgends muss man sich anmelden.</b>",
    'family.db.sub': "Datenbanken",
    'family.db': "Ein Datenbank-Client mit eingebautem MCP-Server — neun Adapter, von MySQL und PostgreSQL bis Turso, D1 und Firestore. <b>Nur lesend, solange Sie eine Verbindung nicht ausdrücklich freigeben</b>, und selbst dann werden Rechtevergabe, Rollen-DDL, <code>TRUNCATE</code> und <code>DROP</code> abgelehnt, was immer Sie einstellen.",
    'family.wa.sub': "Gespräche",
    'family.wa': "Menschen und Agenten in einem Gespräch, direkt von Rechner zu Rechner. <b>Kein zentrales Konto, kein Server zum Betreiben</b> — wer den Schlüssel bekommt, kommt herein.",
  },
  'zh-TW': {
    ...
    'f.issues': 'Issues',
    'h.family': "同一個想法，換個地方",
    'family.intro': "三個工具，一條規矩 —— <b>AI 在你看得見的地方，只在你劃的圍籬裡做事。</b>都免費、都開源、都跑在你自己的機器上。<b>沒有需要註冊的地方。</b>",
    'family.db.sub': "資料庫",
    'family.db': "內建 MCP 的資料庫工具。從 MySQL、PostgreSQL 到 Turso、D1、Firestore，共九種。<b>在你為某條連線明確打開之前只能讀</b>；即使打開，改權限、角色 DDL、<code>TRUNCATE</code> 與 <code>DROP</code> 無論怎麼設定都會被拒絕。",
    'family.wa.sub': "對話",
    'family.wa': "讓人與 AI 坐進同一場對話。沒有中心帳號，也沒有要架的伺服器 —— <b>只有拿到鑰匙的人能進來。</b>",
  },
  'zh-CN': {
    ...
    'f.issues': 'Issues',
    'h.family': "同一个想法，换个地方",
    'family.intro': "三个工具，一条规矩 —— <b>AI 在你看得见的地方，只在你划的围栏里干活。</b>都免费、都开源、都跑在你自己的机器上。<b>没有需要注册的地方。</b>",
    'family.db.sub': "数据库",
    'family.db': "内置 MCP 的数据库客户端。从 MySQL、PostgreSQL 到 Turso、D1、Firestore，共九种。<b>在你为某条连接明确打开之前只读</b>；即使打开，改权限、角色 DDL、<code>TRUNCATE</code> 和 <code>DROP</code> 无论怎么设置都会被拒绝。",
    'family.wa.sub': "对话",
    'family.wa': "让人和 AI 坐进同一场对话。没有中心账号，也没有要搭的服务器 —— <b>只有拿到钥匙的人能进来。</b>",
  },
  ko: {
    ...
    'f.releases': '릴리스',
    'f.issues': '이슈',
    'h.family': "같은 생각을, 다른 자리에서",
    'family.intro': "도구는 셋, 규칙은 하나 —— <b>AI는 사람이 볼 수 있는 곳에서, 사람이 친 울타리 안에서만 일한다.</b> 모두 무료이고 소스가 공개되어 있으며 자기 기계에서 돌아갑니다. <b>가입할 곳이 없습니다.</b>",
    'family.db.sub': "데이터베이스",
    'family.db': "MCP를 품은 데이터베이스 도구. MySQL·PostgreSQL부터 Turso·D1·Firestore까지 아홉 가지. <b>사람이 연결마다 열어 주기 전에는 읽기 전용</b>이고, 연 뒤에도 권한 변경·롤 DDL·<code>TRUNCATE</code>·<code>DROP</code>은 무엇을 적어도 통하지 않습니다.",
    'family.wa.sub': "대화",
    'family.wa': "사람과 AI가 같은 대화에 앉는 도구. 중앙 계정도, 세울 서버도 없습니다 —— <b>키를 건넨 상대만 들어옵니다.</b>",
  },
  ja: {
    ...
    'f.releases': 'リリース',
    'f.issues': 'Issue',
    'h.family': "同じ考えを、別の場所で",
    'family.intro': "道具は 3 つ、決まりは 1 つ —— <b>AI は、人に見える所で、人が引いた囲いの中だけで働く。</b>どれも無料で、ソースが公開されていて、自分の機械で動きます。<b>登録する先がありません。</b>",
    'family.db.sub': "データベース",
    'family.db': "MCP を内蔵したデータベースの道具。MySQL・PostgreSQL から Turso・D1・Firestore まで 9 種。<b>人が接続ごとに開けるまで読み取りだけ</b>で、開けたあとも権限の変更・ロールの DDL・<code>TRUNCATE</code>・<code>DROP</code> は、何を書いても通りません。",
    'family.wa.sub': "会話",
    'family.wa': "人と AI が同じ会話に着く道具。中央のアカウントも、建てるサーバーもありません —— <b>鍵を渡した相手だけが入れます。</b>",
  },
};
