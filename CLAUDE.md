...
1. **`run_command(cmd)` 相当の MCP ツールを作らない。**
   引数で任意の文字列をシェルへ渡し、**人の見ていない所で終わる**口を作らない（D3）。
   必要な操作は用途別ツールか、許可リスト方式の `run_readonly` に足す。
   **端末は例外**（D29）— AI もコンソールを触れるが、**排他ロックと停止ボタン**が付き、
   **人が常に見ていて、いつでも取り返せる**。「投げっぱなし」との違いがここ。
...
| ~~端末を画面で 1 回打ってみる~~ → **2026-09-04 に打たれ、動きませんでした**（Issue #10）。直したので**もう 1 回**打ってほしい | 人 |
| **`readonly.toml` を書いてみる**（実装済・D3。**既定は空なので、書くまで AI は 1 本も走らせられません**） | 人 |
| Windows 実機（001 / 004） | 人 |
| **`become = "ask"` を 1 回押してみる**（実装済・D48。**パスワードの欄が出るか。打ったものがどこにも出ていないか**） | 人 |
| **Issue #8 の根本原因**（画面が実態とずれる）。**まだ分かっていません** | AI |
| 画面そのもののテスト（部品を描いて確かめるもの）。**問い 3 枚で 22 本。ファイルの面と端末の面は手つかず** | AI |
...
### 手段を指定されたとき（product-baseline §30）

**「rc で」「issue に」「MCP から」と言われたら、その口を使う。**

**使えないときは、実行する前に言う** ——
「**rc に相手が出ていません。`/remote-control` を試しますか、別の手段にしますか**」

**勝手に別の手段へ切り替えない。**届き方が違う。
2026-09-24 に、rc を指定されたのに PR コメントへ切り替えた
（**相手が開くまで届かない所へ置いて、「謝罪した」事実だけを作った**）。

**見えない＝無い、と決めない。**実際には繋がっており、`/remote-control` で 14 席出た。
