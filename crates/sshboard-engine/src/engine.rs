...
impl Engine {
    ...
    pub async fn run_operation(&self, actor: Actor, id: &str) -> Result<Ran, EngineError> {
        ...
        // **「届かなかった」と「通らなかった」を分ける**（2026-09-25）。
        //
        // 実機でこれに 1 往復まるごと使いました。`sudo -S` は 1 回目が通らないと
        // **もう一度読みに行き**、閉じた入力で EOF に当たります。だから ——
        //
        // ```
        // 空を食わせた      残念、また試してください。／ パスワードが与えられませんでした
        // 違う字を食わせた  **まったく同じ 3 行**
        // ```
        //
        // **サーバーの返事だけでは区別が付きません。**こちら側で分けます。
        //
        // **長さは書きません。**診断ログは**人が issue へそのまま貼る前提の物**で
        // （接続先を入れない作りもそのため）、**書くとパスワードの長さが
        // 公開の場へ出ます。**有無だけで、知りたいこと（届いたか）は分かれます。
        if let Some(fed) = fed.as_deref() {
            // 末尾の改行はこちらが足したもの。**中身が在るかだけを見ます。**
            if fed.trim_end_matches('\n').is_empty() {
                self.diag.warn(
                    Stage::Exec,
                    None,
                    format!("{id} へ渡す秘密が空でした（人が入れたものが届いていません）"),
                );
            } else {
                self.diag
                    .info(Stage::Exec, None, format!("{id} へ秘密を渡しました"));
            }
        }
